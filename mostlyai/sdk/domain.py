...
#   timestamp: 2025-05-22T08:15:49+00:00
...
class ModelConfiguration(CustomBaseModel):
    """
    The training configuration for the model with simplified but effective optimizations.
    """
    ...
    batch_size: int | None = Field(
        None,
        alias="batchSize",
        description="The physical batch size used for training the model.\nIf not provided, batchSize will be chosen automatically based on available memory and model size.\n",
        ge=1,
    )
    ...
    # Enhanced Training Parameters (Simplified)
    use_mixed_precision: bool | None = Field(
        False,
        alias="useMixedPrecision",
        description="Enable PyTorch native automatic mixed precision training.\n"
        "Provides ~30-50% speedup with minimal memory overhead on compatible GPUs.\n",
    )
    
    gradient_clip_norm: float | None = Field(
        None,
        alias="gradientClipNorm",
        description="Maximum norm for gradient clipping to prevent exploding gradients.\n"
        "Recommended value: 1.0. Set to null to disable clipping.\n",
        gt=0.0,
    )
    ...
    learning_rate: float | None = Field(
        ...
        "If not provided, will be automatically determined based on batch size using sqrt scaling.\n",
        gt=0.0,
    )

    # Learning Rate Scheduling
    scheduler_type: SchedulerType | None = Field(
        SchedulerType.plateau,
        alias="schedulerType",
        description="Learning rate scheduling strategy.\n"
        "- warmup_cosine: Warmup followed by cosine annealing (recommended)\n"
        "- one_cycle: One-cycle learning rate policy with peak in middle\n"
        "- plateau: Reduce LR when validation loss plateaus (conservative)\n",
    )
    
    warmup_epochs: int | None = Field(
        5,
        alias="warmupEpochs",
        description="Number of epochs for learning rate warmup phase.\n"
        "During warmup, learning rate gradually increases from 0 to the initial rate.\n"
        "Only applicable for warmup_cosine and one_cycle schedulers.\n",
        ge=0,
    )
    ...
    min_lr_ratio: float | None = Field(
        0.01,
        alias="minLrRatio", 
        description="Minimum learning rate as a ratio of the initial learning rate.\n"
        "Used as the final learning rate in cosine annealing and one-cycle.\n",
        ge=0.0,
        le=1.0,
    )
    ...
    weight_decay: float | None = Field(
        0.01,
        alias="weightDecay",
        description="Initial weight decay (L2 regularization) coefficient.\n"
        "Helps prevent overfitting by penalizing large weights.\n",
        ge=0.0,
    )
    
    weight_decay_end: float | None = Field(
        None,
        alias="weightDecayEnd",
        description="Final weight decay value for scheduled weight decay.\n"
        "If provided, weight decay will be linearly interpolated from weightDecay to weightDecayEnd.\n"
        "If not provided, defaults to weightDecay * 0.1.\n",
        ge=0.0,
    )
    
    dnn_dropout: float | None = Field(
        0.25,
        alias="dnnDropout",
        description="Dropout rate for dense neural network layers.\n"
        "Applied to embedding, context, and regressor layers to prevent overfitting.\n"
        "Recommended values: 0.1-0.5. Set to 0.0 to disable dropout.\n",
        ge=0.0,
        le=1.0,
    )
    
    lstm_dropout: float | None = Field(
        0,
        alias="lstmDropout",
        description="Dropout rate for LSTM layers.\n"
        "Applied between LSTM layers in multi-layer configurations.\n"
        "Recommended values: 0.0-0.3. Set to 0.0 to disable LSTM dropout.\n",
        ge=0.0,
        le=1.0,
    )

    # Loss Function Enhancements
    use_focal_loss: bool | None = Field(
        False,
        alias="useFocalLoss",
        description="Use focal loss instead of standard cross-entropy loss.\n"
        "Recommended only when dealing with severe class imbalance.\n",
    )
    
    focal_alpha: float | None = Field(
        1.0,
        alias="focalAlpha",
        description="Alpha parameter for focal loss weighting.\n"
        "Only applicable if useFocalLoss is enabled.\n",
        gt=0.0,
    )
    ...
    focal_gamma: float | None = Field(
        2.0,
        alias="focalGamma",
        description="Gamma parameter for focal loss focusing.\n"
        "Higher values focus more on hard examples. Only applicable if useFocalLoss is enabled.\n",
        ge=0.0,
    )
    
    label_smoothing: float = Field(
        0.0,
        alias="labelSmoothing",
        description="Label smoothing factor to prevent overconfident predictions.\n"
        "Recommended value: 0.1. Set to 0.0 to disable smoothing.\n",
        ge=0.0,
        le=1.0,
    )
    ...
    early_stopping_min_delta: float | None = Field(
        1e-4,
        alias="earlyStoppingMinDelta",
        description="Minimum improvement in validation loss to be considered as improvement.\n"
        "Smaller values are more sensitive to small improvements.\n",
        gt=0.0,
    )
